...
- [[#backups--syncing][Backups & Syncing]]
  - [[#backups][Backups]]
    ...
    - [[#auto-restore-cleanup-tag][Auto Restore Cleanup Tag]]
    - [[#rofi-menu][Rofi Menu]]
  - [[#syncthing][Syncthing]]
...
- [[#miscellaneous-scripts][Miscellaneous Scripts]]
  ...
  - [[#gentoo][Gentoo]]
    - [[#main-control-script][Main Control Script]]
...
[submodule "dracula-calibre-theme"]
	...
	branch = main
[submodule "mangowc"]
	path = mangowc/.config/mango
	url = ../mangowc.git
	branch = main
[submodule "dracula-beeper"]
	path = beeper/.themes/dracula-beeper
	url = git@github.com:dracula/beeper.git
#+END_SRC
...
!fish/.config/fish/themes/Dracula_Official.theme
...
[
    ...
    {
        // Disable command palette keybinding conflict in vim
        "key": "ctrl+p",
...
configs:
  ...
  - name: root
    ...
    repo: /mnt/5TB_HDD/Backups/powerspec/root
...
configs:
  - name: home
    # MANDATORY: borg repo target, e. g. backupuser@backuphost:reponame
    repo: /mnt/5TB_HDD/Backups/powerspec/home
    # if this is set to true, borg does not neccessarily fail when a backup fails
    fault_tolerant_mode: true
    # snapborg fails when the most recent snapshot transferred successfully is
    # older than the time period given here. Set to '0d' to disable this behaviour
...
configs:
  ...
  - name: root
    # MANDATORY: borg repo target, e. g. backupuser@backuphost:reponame
    repo: /mnt/fileserver/Backups/powerspec/root
    # if this is set to true, borg does not neccessarily fail when a backup fails
    fault_tolerant_mode: true
    # snapborg fails when the most recent snapshot transferred successfully is
    # older than the time period given here. Set to '0d' to disable this behaviour
...
configs:
  - name: home
    # MANDATORY: borg repo target, e. g. backupuser@backuphost:reponame
    repo: /mnt/fileserver/Backups/powerspec/home
    # if this is set to true, borg does not neccessarily fail when a backup fails
    fault_tolerant_mode: true
    # snapborg fails when the most recent snapshot transferred successfully is
    # older than the time period given here. Set to '0d' to disable this behaviour
...
*** Auto Restore Cleanup Tag

Snapper creates snapshots with cleanup policy set, but Snapborg modifies the snapshot metadata to mark it as backed up.
In the process, it inadvertently drops the cleanup policy. This script fixes affected snapshots after Snapborg runs.

#+BEGIN_SRC shell :shebang #!/usr/bin/env bash :tangle snapborg/.scripts/fix-snapborg-cleanup.sh
...
#+BEGIN_SRC systemd :tangle waybar/.config/systemd/user/waybar.service.d/override.conf
[Service]
ExecStart=
ExecStart=/usr/bin/waybar --config %h/.config/<<compositor-config-name>>/waybar/config.jsonc --style %h/.config/<<compositor-config-name>>/waybar/style.css
#+END_SRC
...
#+BEGIN_SRC shell :shebang #!/usr/bin/env bash :tangle uwsm/.scripts/session.sh
help_menu() {
...
#+BEGIN_SRC shell :shebang #!/usr/bin/env bash :tangle gentoo/.scripts/.gentoo_update_packages.sh
set -euxo pipefail
...
#+BEGIN_SRC shell :shebang #!/usr/bin/env bash :tangle gentoo/.scripts/.gentoo_update_kernel_config.sh
set -euxo pipefail
...
#+BEGIN_SRC shell :shebang #!/usr/bin/env bash :tangle gentoo/.scripts/.gentoo_spice_up_spotify.sh
set -euxo pipefail
